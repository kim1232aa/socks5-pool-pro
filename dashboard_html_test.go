...
func TestDashboardPrivateSourceEscapeHatchIsExplicitAndOffByDefault(t *testing.T) {
	...
	if strings.Contains(dashboardHTML, `id="source-allow-private" name="allow_private" type="checkbox" checked`) {
...
func TestDashboardProxyIPVerifyIsExplicitResourceOnlyAction(t *testing.T) {
	...
	if got := strings.Count(dashboardHTML, "fetchJSON('/api/proxyip/verify'"); got != 1 {
		t.Fatalf("ProxyIP verify endpoint call count = %d, want one explicit action path", got)
	}
	guard := strings.Index(dashboardHTML, "if (String(protocol || '').toLowerCase() !== 'proxyip')")
	button := strings.Index(dashboardHTML, `data-action="proxyip-verify"`)
...
func TestDashboardCandidateVerifyColumnKeepsTableShape(t *testing.T) {
	if !strings.Contains(dashboardHTML, `<thead><tr><th>状态</th><th>协议</th><th>候选地址</th><th>来源标注地区</th><th>来源</th><th>专用验证</th></tr></thead>`) {
		t.Fatal("candidate verify header is missing")
	}
	if strings.Contains(dashboardHTML, `colspan="5"`) {
		t.Fatal("candidate empty rows still use the old five-column colspan")
	}
	if got := strings.Count(dashboardHTML, `colspan="6"`); got != 3 {
...
func TestDashboardManualVerifyShowsDebouncedHealthObservation(t *testing.T) {
	...
	if strings.Contains(dashboardHTML, `可能已失效`) {
...
func TestDashboardManualVerifyHandlesUnknownLabelMatch(t *testing.T) {
	...
	unknownIndex := strings.Index(dashboardHTML, unknownGuard)
	legacyIndex := strings.Index(dashboardHTML, legacyGuard)
