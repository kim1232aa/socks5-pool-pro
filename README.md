...
> `/api/status` 保留现有提取客户端和注册服务依赖的兼容合同：顶层继续包含 `active_proxy`、`available_total` 和仅包含健康节点的 `proxies`；每项包含带认证的 `proxy_url`、原始 `username`、`password`，SOCKS5 节点额外有 `socks_url`。状态 API **不会返回 `telegram_url`**。
...
管理界面可覆盖每轮检测并发、单节点超时、每轮抽样规模、来源刷新周期、完整全池复检周期和 `-require-ip-change`。这些覆盖持久化在 `pool_config.json`；两个周期独立保存，`0` 表示跟随各自 CLI 默认，运行时修改会在下一轮计时器重置时生效而无需重启。要求改 IP 策略变化会刷新基线出口、立即使旧健康结果失效并触发全池复检，选择“跟随命令行默认”则恢复 CLI 值。

候选自动批检默认开启：独立后台 worker 在来源刷新之外持续轮转待检候选，每批复用每轮抽样规模、并发、超时与要求改 IP 设置；“批间暂停”可调（`0` 为连续检测），关闭开关后只保留来源刷新周期内的那一批。自动批检只领取候选租约，失败节点仍只能手动重测——失败节点页除了勾选重测，还提供“一键重查全部失败节点”：服务端枚举全部失败记录，按每轮抽样规模分批在单个长任务内走完，进度可在任务状态中查看；管理任务槽同一时间只允许一个人工任务。

基线出口也可手动刷新。程序通过无环境代理、拒绝重定向的直接 HTTPS 客户端，在同一总超时内并发竞速 `api64.ipify.org`、`checkip.amazonaws.com` 与 `ident.me`；首个合法 IP 胜出并取消其余请求。全部端点失败时保留上一条基线；基线真实变化且策略启用时，同样会失效旧结果并全池复检。
...
新候选只有成功后才进入已知转发池。一次轻量自动检查或人工验证在所有尝试后仍失败，会把已知节点标记为 terminal 并立即停止路由；有冷却限制的轻量检查不会再选它，但完整全池复检仍包含可用、不可用和 terminal 节点，成功时会恢复可用状态并清零连续全检失败计数。节点连续三轮完整全检失败后，才会自动移出转发池并删除对应统计；候选目录记录不删除，未来来源刷新仍可重新发现和验证它。取消或被新健康代次取代的未完成工作不计失败。实际转发时若确认是上游连接、认证或协议故障，也会立即停止选择该节点，而目标站自身拒绝连接不会污染全局节点健康。管理员仍可显式“永久清理不可用节点”。

同一个协议与地址如果由来源声明了多组凭据，会保留有界的凭据候选，并在同一个节点超时预算内逐个尝试；验证成功的凭据会提升为当前主凭据。同一 `IP:port` 的 SOCKS5 与 HTTP 声明仍是两个独立节点，不共享错误状态。

### 测速不是健康检查

测速是按需操作，不会在每轮刷新中对全池自动下载。候选节点（尚未在池里的节点）的发起测速也会被拦截并强行前置当前健康标准检查：候选必须先对当前配置的 check_url 连通并在 Exit IP 策略通过后，才能发起真实下载测速；如果不健康，直接报 candidate_healthcheck_failed 并拒绝进入选路。健康节点测速完整读取固定 1 MiB 样本，拒绝重定向、非完整响应和异常 Range；主测速点失败后会在同一 18 秒总预算内尝试备用测速点。测速与人工验证分别允许最多 16 个不同节点并发执行，不进入隐藏队列；同一节点测速完成后冷却 5 分钟，人工验证完成后冷却 2 分钟。轻量自动健康检测成功后 24 小时内不会重复检测；最终失败的节点立即从可路由池过滤，但仍会进入独立周期的完整全池复检，也可由显式人工验证成功恢复。测速失败可能只是测速站链路问题，不会因此直接判定代理不健康。`speed` 策略只对已有有效测速样本的节点有参考意义。
...
| GET/POST | `/api/settings/check-options` | 读取或修改检测选项、来源刷新/全检周期和要求改 IP 覆盖 |
...
| `-data-dir` | `./data` | 持久化目录 |
| `-scrape-interval` | `20m` | 来源刷新 CLI 默认周期 |
| `-full-recheck-interval` | `30m` | 完整转发池复检 CLI 默认周期 |
...
| `SCRAPE_INTERVAL` | `20m` | 来源刷新 CLI 默认周期 |
